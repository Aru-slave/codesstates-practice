...
public class KDeliveryMain {
    ...
    public void selectAddShopMenu() {
        ...
        System.out.print(">>>");
        int price = s.nextInt();
        Shop shopMenu = new Shop(shopName);

        System.out.println(String.format("[안내]%s에 음식(%s, %d) 추가되었습니다.", shopName, food, price));
        shopMenu.addFood(shopIdx, food, price);
        shops[shopIdx] = shopMenu;
        shopIdx++;
    }
    ...
    void selectDashboardMenu() {
        for(int i = 0; i < feedbacks.length; i++){
            if(feedbacks[i] != null){
                System.out.println(String.format("%s 고객님",feedbacks[i].getCustomerName()));
                System.out.println(String.format("주문 매장 : %s",feedbacks[i].getShopName()));
                System.out.println(String.format("주문 메뉴 : %s",feedbacks[i].getFoodName()));
                System.out.print("별점 : ");
    ...
    void selectOrderMenu() {
        ...
        System.out.print(">>>");
        String food = s.next();
        Order order = new Order(customerName, shopName, food);
    ...
    void selectFeedbackMenu() {
        ...
        System.out.print(">>>");
        String shopName = s.next();
        System.out.println("[안내] 주문하신 음식 이름이 무엇인가요?");
        System.out.print(">>>");
        String food = s.next();
        System.out.println("[안내] 음식맛은 어떠셧나요? (1~5)점");
        System.out.print(">>>");
        int grade = s.nextInt();
        Feedback review = new Feedback(name, shopName, food, grade);
        feedbacks[feedBackIdx] = review;
        feedBackIdx++;
    }

    public static void main(String[] args) {
        KDeliveryMain main = new KDeliveryMain();
        do {
            switch (main.selectMainMenu()) {
                case 1:
                    main.selectAddShopMenu();
                    break;
                case 2:
                    main.selectDashboardMenu();
                    break;
                case 3:
                    main.selectOrderMenu();
                    break;
                case 4:
                    main.selectFeedbackMenu();
                    break;
                case 5:
                    main.close();
            }
